...
from .sub_agents.client_agent.agent import client_agent
from .sub_agents.contract_accounting_agent.agent import contract_accounting_agent
...
tenzing_agent = Agent(
    ...
    **Interaction History:**
    ...
    4. Client Agent  Tool
       -For questions related to customer contracts, negotiations, retentions, clauses, 
       clients financial agreements such as share percentage, exclusions, safety margins, exposure, 
       pricings and submissions, curves analysis etc.      
       - If there is a mention of any customers specially **Sunset Sky**, **Solid Earth** or **Ocean Storm**,
       Please use only client Agent Tool. This is very important.

    5. Contract Accounting Agent Tool
       - Use this tool to answer queries related to specific contracts
       - This tool should be used if the customer queries for a particular contract number.
       - Format of Contract number is TP10023480-2024
   ...
   #  tools=[check_authorization, AgentTool(ezflow_agent), AgentTool(general_agent)],
    tools=[check_authorization, AgentTool(ezflow_agent), AgentTool(general_agent), AgentTool(client_agent), AgentTool(contract_accounting_agent)],    
)