...
exclude_people = (
    ...
    'exclusion', 'construnction', 'renew', 'ocean', 'premiums', 'upper', 'surplus', 'share', 'main', 'offshore', 'occ', 'sudden', 'roof', 'accounting',
    'kind', 'control', 'the', 'addendum', 'company', 'email', 'law', 'limited', 'mar', 'office', 'sanction', 'stair', 'sums', 'vml'
    )
...
def anonymize_and_write_emails(input_path, output_path):
    msg = Message(input_path)
    msg_sender = obfuscate_entities(decode_if_bytes(msg.sender))
    msg_subject = obfuscate_entities(decode_if_bytes(msg.subject))
    msg_date = decode_if_bytes(msg.date)
    msg_to = obfuscate_entities(decode_if_bytes(msg.to))
    msg_cc = obfuscate_entities(decode_if_bytes(msg.cc))

    # Get the HTML body or fallback to plain text
    msg_body_html = msg.htmlBody or msg.body
    msg_body_html = decode_if_bytes(msg_body_html)

    # Replace names using regex
    msg_body_html = obfuscate_entities(msg_body_html)

    # Build HTML email content
    new_eml_content = f"""From: {msg_sender}
To: {msg_to}
Cc: {msg_cc}
Subject: {msg_subject}
Date: {msg_date}
...
"""
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(new_eml_content)

    # convert_eml_to_pdf(output_path, output_path.replace('.eml', '.pdf'))
    # convert_eml_to_pdf_no_admin(output_path, output_path.replace('.eml', '.pdf'))

    print(f"Modified email saved to {output_path}")
...
def anonymize_emails():
    ...
    for input_file in input_files:
        ...
        output_eml_file = output_eml_file.replace('.msg', '.eml')
        print(input_file, ' ===>', output_eml_file)
        anonymize_and_write_emails(input_file, output_eml_file)
